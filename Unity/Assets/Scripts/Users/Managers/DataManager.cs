...
public class DataManager : MonoBehaviour
{
    ...
    public User loginUserInfo;      // 로그인한 유저 정보

    [Header("ChannelInfo")]
    public int channelIndex = -1;        // 접속 채널 인덱스
    public string channelName;      // 접속 채널 이름
    public int cnt;                 // 채널 참가인원
    public bool isOnGame;           // 게임중 여부
    public List<SessionData> sessionList;   // 참가중인 유저 정보(players와 인덱스 공유)
    public int clearUserId = -1;         // 클리어 한 유저 아이디

    [Header("Player")]
    public int myIdx = -1;   // sessionList중 나의 인덱스
    public List<GameObject> players;   // 플레이어들 정보(sessionList와 인덱스 공유)
    public int[] score; // 인덱스별 점수
    //public List<WeaponManager> weaponManagerScript; // 팔 매니저(sessionList와 인덱스 공유)
